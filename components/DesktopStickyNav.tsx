...
export function DesktopStickyNav() {
  ...
  return (
    <div 
      className={`hidden md:flex fixed bottom-8 left-1/2 -translate-x-1/2 z-[9999] transition-all duration-500 ease-in-out transform ${
        isVisible ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0 pointer-events-none"
      }`}
    >
      <div className="flex items-center gap-4 bg-white/80 backdrop-blur-md p-2 rounded-full shadow-2xl border border-white/20">
        <ProductDropdown gender="men">
          <a 
            href={productsLink} onClick={trackProductsClick} 
            className="bg-[#112255] text-white font-bold text-lg uppercase tracking-wide px-8 py-3 rounded-full flex items-center gap-3 shadow-lg hover:scale-105 transition-transform group border-2 border-transparent hover:border-blue-200/30"
          >
        ...
        <ProductDropdown gender="women">
          <a 
            ...
            className="bg-[#D65A8A] text-white font-bold text-lg uppercase tracking-wide px-8 py-3 rounded-full flex items-center gap-3 shadow-lg hover:scale-105 transition-transform group border-2 border-transparent hover:border-pink-200/30"
          >
